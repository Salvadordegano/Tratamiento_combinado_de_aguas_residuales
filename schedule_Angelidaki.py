...
from schedule_ASM import dbp as dbp_ae
from schedule_ASM import tiempo as tiempo_ae
...
y0_TD = [2.2500000000041034, 0.11579206982516568, 0.14487541492125505, 2.359011464187238e-06, 3.1537123630259086e-05, 2.2891860941681573e-22, 4.7336386786414537e-08, 2.9051411844137166e-06, 0.0005063603096411737, 1.613481241603337e-05, 3.7008395260591168e-06, 0.00020893179370781218, 0.00014413212498234465, 1e-12, 1e-12, 0.014051206373001564, 1e-06, 0.008248946509023954, 0.0006692302166354556, 0.0008338350099709444, 0.001414944901984964, 0.003634935021594099, 0.0005060298592896376, 0.000873235054988352, 0.0007406482951924132, 0.004465092159699516, 0.6286126464487666, 2.0328241225287442, 2.5328203662730875, 4.297974086081511, 11.041317937837666, 1.5370939313739098, 2.6525002014239343, 2.2497605208959586, 13.562966563401645, 0.025, 0.005, 0.0, 1e-12, 1e-12, 1e-12, 1e-12, 1e-12, 1e-12]
...
y0_TEA = [0.01, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1E-12, 1e-12, 1e-12, 1e-12, 1e-12, 1e-12, 1e-12, 1e-12]
...
ctes_TD = [0.5, 0.2, 0.95, 0.05, 0.41, 0.32, 0.27, 0.1, 0.13, 0.12, 0.65, 0.71, 0.29, 0.57, 0.29, 0.14, 0.81, 0.19, 0.59, 0.41, 0.18, 0.82, 0, 0, 0.00625, 0.009, 0.0220, 0.0313, 0.032, 0.0217, 0.0240, 0.0250, 0.0268, 0.0313, 0.01, 0, 0, 1026, 30, 87.5, 0.035]
# [qby, qout]
ctes_TS = [0, 264]
# [qin, qout]
ctes_TEA = [0, 0]
...
def continuar(parameters, constants, inlet, t_inicial, t_final, initial):
    ...
    tanquedigestor = an.Tanque_Digestor(parameters, constants[1], inlet[0], initial[1], [t_inicial, t_final], np.linspace(t_inicial, t_final, int((t_final-t_inicial)/0.001)))
    ...
    dbp.append(td_resultado[36])
    
    # Saving new initial conditions
    return [y0_TS, y0_TD, y0_TEA, inlet_td, inlet_tea]

while t_final < 4:
    y0_TS, y0_TD, y0_TEA, feed_TD, feed_TEA = continuar(pars_TD, [ctes_TS, ctes_TD, ctes_TEA], [feed_TD, feed_TEA], t_inicial, t_final, [y0_TS, y0_TD, y0_TEA]) #Llenado
    t_inicial = t_final
    ...
    ctes_TEA[1] = 0
    y0_TEA[0] = 0.01
    y0_TS, y0_TD, y0_TEA, feed_TD, feed_TEA = continuar(pars_TD, [ctes_TS, ctes_TD, ctes_TEA], [feed_TD, feed_TEA], t_inicial, t_final, [y0_TS, y0_TD, y0_TEA]) #Reacción
    t_inicial = t_final
    t_final = t_inicial + 0.5/24
    ctes_TD[36] = 0
    y0_TS, y0_TD, y0_TEA, feed_TD, feed_TEA = continuar(pars_TD, [ctes_TS, ctes_TD, ctes_TEA], [feed_TD, feed_TEA], t_inicial, t_final, [y0_TS, y0_TD, y0_TEA]) #Decantación
    t_inicial = t_final
    t_final = t_inicial + 0.25/24
    ctes_TD[23] = 264
    y0_TS, y0_TD, y0_TEA, feed_TD, feed_TEA = continuar(pars_TD, [ctes_TS, ctes_TD, ctes_TEA], [feed_TD, feed_TEA], t_inicial, t_final, [y0_TS, y0_TD, y0_TEA]) #Vaciado
    t_inicial = t_final
    t_final = t_inicial + 0.25/24
    ctes_TD[23] = 0
    ctes_TS[1] = 264
...
data_nat = pd.read_excel(r'Datos.xlsx', sheet_name='NAT (2)')
biogas_angelidaki = pd.DataFrame(data_biogas, columns=['Biogas [L]'])
metano_angelidaki = pd.DataFrame(data_metano, columns=['CH4 [L]'])
nat_angelidaki = pd.DataFrame(data_nat, columns=['NAT [mg/L]'])
tiempo_nat_angelidaki = pd.DataFrame(data_nat, columns=['Tiempo [d]'])
tiempo_angelidaki = pd.DataFrame(data_biogas, columns=['Tiempo [d]'])

tiempo = [i for sublist in tiempo for i in sublist]
...
"""
fig, ax = plt.subplots()
for t_seg, p_seg in zip(tie, pH):
    ax.plot(t_seg, p_seg, '-', color="orange", linewidth=2.5)

ax.set_xlabel('Tiempo, d', fontsize=15)
ax.set_ylabel(r'$I_{AGCL}$', fontsize=15)
plt.show()


fig = plt.figure(figsize=(10, 8))
plt.subplot(2,2,1)
plt.plot(tiempo, [i for sublist in nat for i in sublist],'y-',linewidth=3)
plt.plot(tiempo_nat_angelidaki, nat_angelidaki, 'o', color= "green", ms=5)
plt.ylabel('pH')
...
plt.plot(tiempo, [i for sublist in metano for i in sublist], 'g-',linewidth=3)
plt.ylabel('gCOD/L')
...
fig = plt.figure()
plt.plot(tiempo, [i*100 for sublist in dbp for i in sublist], '-', color="orange", linewidth=2.5)
plt.plot([i for sublist in tiempo_ae for i in sublist], [i*100 for sublist in dbp_ae for i in sublist], '-', color="yellow", linewidth=2.5)
plt.ylabel('Diámetro promedio de gránulo, mm', fontsize=15)
plt.xlabel('Tiempo, d', fontsize=15)
plt.ylim(2, 4)
#plt.legend(['AGCL'],loc='best')
plt.show()

"""

fig = plt.figure()
plt.plot(tiempo, [i for sublist in lcfa for i in sublist],'-', color= "orange", linewidth=3)
#plt.plot(tiempo_nat_angelidaki, nat_angelidaki, 'o', color= "green", ms=5)
plt.ylabel('AGCL, mg L' + r'$^{-1}$', fontsize=15)
plt.xlabel('Tiempo, d', fontsize=15)
plt.show()